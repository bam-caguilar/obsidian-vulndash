...
test('builds NVD fetch requests with explicit modified-date params', () => {
  const builder = new NvdRequestBuilder();
  const request = builder.buildFetchRequest({
    startIndex: 0,
  ...
  assert.match(request.url, /lastModStartDate=2026-04-20T00%3A00%3A00.000Z/);
  assert.match(request.url, /lastModEndDate=2026-04-20T23%3A59%3A59.999Z/);
});
